...
| **optimization** | **game** |
| --- | --- |
| min<sub>x∈X</sub> f(x) | min<sub>x<sub>i</sub> ∈ X<sub>i</sub></sub> f<sub>i</sub>(x<sub>i</sub>, x<sub>-i</sub>), i ∈ [N]
| (strong) convexivity | (strong) monotonicity |
| **(cvx)** global minimizer | **(cvx)** NE: f<sub>i</sub>(x<sub>i</sub>*, x<sub>-i</sub>*) ≤ f<sub>i</sub>(x<sub>i</sub>, x<sub>-i</sub>*) for i ∈ [N]
...
>> ○ **Time-invariant**: The reward r<sub>t</sub>(s<sub>t</sub>, a<sub>t</sub>, s<sub>t+1</sub>) is known and, if infinite-horizon case, time-invariant.
...
> ① Since s<sub>t+1</sub> is ambiguous, we allow the reward at time t to depend on s<sub>t+1</sub> as well: r<sub>t</sub>(s<sub>t</sub>, a<sub>t</sub>, s<sub>t+1</sub>) instead of r<sub>t</sub>(s<sub>t</sub>, a<sub>t</sub>).
...
<img width="719" height="504" alt="스크린샷 2025-12-14 오전 2 47 43" src="https://github.com/user-attachments/assets/60a68c55-b2d1-4e5c-9611-7b540e2e6086" />
...
<img width="702" height="483" alt="스크린샷 2025-11-30 오후 8 37 20" src="https://github.com/user-attachments/assets/7079809d-ddb9-4db7-a01f-410ca7bfa989" />
...
| L<sub>2</sub> (χ<sup>2</sup>-type) set  | 𝒪(ㅣSㅣ) | 𝒪(ㅣSㅣlogㅣSㅣ) |
| L<sub>1</sub> set | 𝒪(ㅣSㅣ) | 𝒪(ㅣSㅣlogㅣSㅣ) |
...
> ② The optimization inf<sub>p∈𝒫</sub> 𝔼<sup>p</sup>[v] can be solved in 𝒪(ㅣ𝒮ㅣ log(ㅣ𝒮ㅣ)) time.

⑷ **Application 3.** L<sub>1</sub> Approximation Sets

> ① D(p ㅣㅣ q) ≥ (1 / 2 ln 2) ㅣㅣp - qㅣㅣ<sub>1</sub><sup>2</sup> → 𝒫 = {p: ㅣㅣp - qㅣㅣ1 ≤ δ}, δ = √(2t ln2) used.

> ② L<sub>1</sub> / L<sub>∞</sub> type sets are convenient for modeling but weak as statistical confidence regions → in practice, L<sub>2</sub>-based sets are more recommended.
