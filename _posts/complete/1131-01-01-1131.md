...
> ① Since s<sub>t+1</sub> is ambiguous, we allow the reward at time t to depend on st+1 as well: r<sub>t</sub>(s<sub>t</sub>, a<sub>t</sub>, s<sub>t+1</sub>) instead of r<sub>t</sub>(s<sub>t</sub>, a<sub>t</sub>).
...
| ----------------- | --------------------------- | ---------------------------- | 
| KL confidence set | (O(ㅣSㅣ)) | (O(ㅣSㅣ)) (increase in times of a constant) |
| L<sub>2</sub> (χ<sup>2</sup>-type) set  | (O(ㅣSㅣ)) | (O(ㅣSㅣ log ㅣSㅣ)) |
| L<sub>1</sub> set | (O(ㅣSㅣ)) | (O(ㅣSㅣ log ㅣSㅣ)) |
