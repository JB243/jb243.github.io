...
Since $\lambda\neq 0$, this would imply $a=b=c$, which is a contradiction.

Therefore, at least two variables must be equal. Without loss of generality, we may assume $a=b.$ In this case, $c$ can be expressed as a function of $a$, so the desired inequality reduces to a one-variable problem. The maximum is then easily obtained. The equality condition is $a=b=c=1.$
