...
> ① **True Positive** (TP) = #TP = 7 + 8 + 9 = 24.

> ② **False Positive** (FP) = #FP = 1 + 2 + 3 + 4 + 5 + 6 = 21.

> ③ **True Negative** (TN): Correctly predicting a negative example as a different class.

> ④ **False Negative** (FN): Incorrectly predicting a negative example as the same class.
...
> ⑩ **False Positive Rate (FDR):** FP ÷ (TN + FP).

> ⑪ **F<sub>1</sub> Score** = 2 × Precision × Recall ÷ (Precision + Recall) = #TP ÷ [ #TP + (#FN + #FP)/2 ].
...
⑺ [Receiver Operating Characteristic (ROC) Curve](https://jb243.github.io/pages/1634)
...
> ② ROC Curve: A graph visualizing sensitivity on the vertical axis and 1-specificity (FDR) on the horizontal axis.
