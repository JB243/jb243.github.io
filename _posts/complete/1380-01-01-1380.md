...
>> ○ Even other reactions tend to have limited applications of **Type 2**
...
> ③ **Type 3:** Enolate and 1,2-Addition Reaction: Occurs when the nucleophile is a strong nucleophile (e.g., Grignard reagent)
...
> ④ **Type 4:** Enolate and 1,4-Addition Reaction: Occurs when the nucleophile is a weak nucleophile (e.g., MeSH)
...
>> ○ Carboxylic acids do not perform acetal formation, so Fischer esterification and **competitive reaction** do not occur
...
>>>> ○ Typically, protocols add EDC and NHS simultaneously to the target molecule solution
...
>>>> ○ Due to its poor water solubility, NHS is sometimes used with DMSO, DMF

>>>> ○ For reactions where water solubility is crucial, sulfo NHS is used[^2^]
...
>> ○ LiAlH<sub>4</sub>: Converts aldehyde to alcohol via intermediate aldehyde
...
> ④ Double deprotonation of carboxylic acids followed by alkylation at the alpha position: 1. NaH, 55 ℃, THF, 2. LDA, 10-35 ℃
...
> ⑧ [Cyanide Hydrolysis of RCN](https://jb243.github.io/pages/1380)
...
>> ○ The reaction is not step-by-step, so some amine is used in the work-up step: Three states exist.
...
> ② **Reaction 2-2.** Reaction with LiAlH(O-t-Bu)<sub>3</sub> to generate aldehydes: 1. LiAlH(OC(CH<sub>3</sub>)<sub>3</sub>)<sub>3</sub>, 2. H<sub>2</sub>O
...
>> ○ Doesn't convert to acyl chloride when treated with NaCl
...
> ① **Reaction 1-1.** Acid-catalyzed hydrolysis reaction: Also capable of reverse reaction, has specific equilibrium state.
...
>> ○ Reaction proceeds due to concentration conditions from pH causing no reverse reaction
...
> ⑤ **Reaction 1-5.** Formation of polymers with cyclic esters: Ring-opening reaction occurs iteratively

⑶ **Reaction 2.** Reduction reaction

> ① **Reaction 2-1.** Alcohol formation reaction with LAH (LiAlH<sub>4</sub>)

>> ○ Less electrophilic than aldehydes and ketones, not completely reduced by NaBH4
...
>> ○ Only reduces aldehydes to a certain extent at low temperatures
...
> ① **Reaction 3-1.** Reaction with organometallic reagents

>> ○ Use sufficient organic reagent to ensure complete reaction since esters, ketones, and alcohols coexist

>> ○ Requires an excess of 2 equivalents of organic reagent for complete reaction, slightly exceeding that in practice
...
> ① Primary and secondary amides can form tautomers through intermolecular hydrogen bonding
