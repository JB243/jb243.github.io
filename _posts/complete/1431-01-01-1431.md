...
>> ○ dCas9 (nuclease-deficient Cas9, dead Cas9): It cannot cut DNA, but it can bind to DNA guided by sgRNA.

>> ○ sgRNA/dCas9-BFP-KRAB complex → binds to the target promoter → recruits the KAP1 complex → heterochromatin formation → gene expression repression

>> ○ Perturbation screening can be performed using different guide RNAs.
...
⑶ [Genome-Shuffle-seq](https://pmc.ncbi.nlm.nih.gov/articles/PMC11931979/)

> ① An experimental technique that generates large-scale structural variants (SVs) across the genomes of mammalian cells and uses sequencing to identify which variants are present in individual cells.
...
<img width="224" height="300" alt="스크린샷 2026-06-23 오전 12 17 07" src="https://github.com/user-attachments/assets/56869679-622a-41cf-ba7d-c5d25229d6d7" />

**Figure 4.** Tn5 tagmentation
...
>> ○ Other techniques to investigate the genome structure

>>> ○ Image-based: MERFISH, DNA seqFISH+, OligoSTORM

>>> ○ Sequencing-based: Micro-C, GAM, SPRITE 
...
> ③ Related altorighms: EagleC, HiNT, C Origami
...
⒁ Spatial epitope sequencing

⒂ Spatial-Mux-seq: RNA, protein, chromatin accessibility and histone modification

⒃ RAISIN-seq(ribosome-bound RNA-seq)

⒄ MIRCL-seq(label-free rare cell enrichment)

⒅ Epi-retro-seq

⒆ [GIFT](https://www.biorxiv.org/content/10.64898/2026.04.11.717967v1.full#F4): Sequencing to read both SNP and RNA from single cell

⒇ [PerturbFate](https://www.nature.com/articles/s41586-026-10367-0): Nascent RNA + Pre-existing RNA + Chromatin accessibility + sgRNA

⒇ SNARE-seq, SHARE-seq, sci-CAR, scCAT-seq: RNA + chromatin accessibility 

⒇ scM&T-seq, snmCT-seq: RNA + DNA methylation

⒇ G&T-seq, DR-seq: Genomic DNA + RNA

⒇ CITE-seq, REAP-seq, ASAP-seq, DOGMA-seq: RNA + protein

⒇ TEA-seq: RNA + chromatin accessibility + protein
