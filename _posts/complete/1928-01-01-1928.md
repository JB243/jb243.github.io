...
>> ○ **Advanced:** #{Eigen(P<sub>n×n</sub>v<sub>n×1</sub>w<sub>1×n</sub>Q<sub>n×n</sub>)} = #{Eigen(w<sub>1×n</sub>Q<sub>n×n</sub>P<sub>n×n</sub>v<sub>n×1</sub>)} = 1, rank = 1
...
>> ○ **Theorem 7.** If $M$ is diagonalizable, then $\log \det M = \operatorname{Tr}(\log M).$

>>> M = PDP<sup>-1</sup> 

>>> det M = λ<sub>1</sub>λ<sub>2</sub> ··· λ<sub>n</sub> ⇒ log det M = ∑<sub>i</sub> log λ<sub>i</sub> = Tr(log D) = Tr(log DP<sup>-1</sup>P) = Tr(log PDP<sup>-1</sup>) = Tr(log M)
