...
>> ○ If `/bin/bash` does not run properly or the `conda` command is not recognized:

>> ○ **Step 1.** Check whether the conda path is included in your PATH.

>>> ○ `echo $PATH | grep miniconda`

>>> ○ `export PATH="/root/miniconda3/bin:$PATH"`

>> ○ **Step 2.** Reload your `~/.bashrc` settings or start a new bash session.
...
⑻ `venv`: installs the environment in a directory, and when you deactivate or remove the `venv`, all related packages are removed as well. ([ref](https://github.com/winterrykim/Site4Drug_Demo/tree/main))
...
⑿ `basilisk`: R package. Supports R-python interoperability ([ref](https://bioconductor.org/books/release/OSTA/pages/bkg-python-interoperability.html#basilisk))
...
> ① The container runtime originally known as Singularity has been renamed to Apptainer.

> ② A `.sif` file refers to a Singularity container image file.

> ③ **Command 1.** Create container

>> ○ `singularity create container.img`

>> ○ An example of Singularity Recipe: It specifys and containes installation software, environment variables, files, and container metadata.
...
⒃ Other container platform ([ref1](https://www.cloudzero.com/blog/docker-alternatives/), [ref2](https://www.saashub.com/singularity-container-alternatives), [ref3](https://octopus.com/blog/top-8-container-registries), [ref4](https://bluelight.co/blog/how-to-choose-a-container-registry)) 
...
> ⑨ Red Hat OpenShift – Kubernetes, Red Hat Quay 
