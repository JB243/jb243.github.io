...
<span style="color: #ffffff; background-color: #ee2323; font-family: Times New Roman;"><b>►</b></span> [Group Theory Lecture](https://www.youtube.com/watch?v=eHXcQGTBgtE&t=192s) 
...
>> ○ [Enormous theorem](https://en.wikipedia.org/wiki/Classification_of_finite_simple_groups): Every finite simple group is classified as cyclic, alternating, Lie group, or sporadic(27 subtypes).

<br>

<img width="724" height="197" alt="스크린샷 2026-06-02 오후 4 13 29" src="https://github.com/user-attachments/assets/09689d7a-ed2c-4240-99fe-bf6c9495cbc8" />

**Figure 2.** Examples of Lie groups
...
>> ○ **Example 1.** CNNs assume _translation invariance_: shifting an image slightly shouldn't change the prediction.

>> ○ **Example 2.** GNNs assume _permutation invariance_: the order of nodes in a graph shouldn't affect the output.

>> ○ **Example 3.** Transformers are _permutation invariant_ by default: they ignore input order unless positional encodings are added. This gives them flexibility, but it also means they rely on you to encode any sequential structure.
...
⑹ **Theorem 3.** Cayley's theorem: every group of size $n$ is a subgroup of the symmetric group $S_n$.

⑺ **Theorem 4.** Maschke's theorem

> ① If the characteristic of a field does not divide the order of a group, then every representation of a finite group decomposes into a direct sum of irreducible representations.

> ② Example: A finite group representation over the complex numbers can always be decomposed into a direct sum of irreducible representations, as
...
> ① [The UVW Principle](https://jb243.github.io/pages/1133): Under a symmetric condition in three variables, applying the method of Lagrange multipliers to a three-variable inequality ultimately shows that an extremum must occur when at least two of the three variables are equal.
...
> ② [Turkevici’s Inequality and Group Theory](https://jb243.github.io/pages/1143)

⑹ By the properties of cyclic groups, the number of elements of exact order $d$ is $\varphi(d)$, where $\varphi$ denotes Euler’s totient function.
