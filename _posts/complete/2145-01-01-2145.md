...
![image](https://github.com/user-attachments/assets/508c80c7-c812-427e-a1c6-f2f3e1e047ae)
...
>> ○ **Feature 1.** asymmetry **:** D<sub>KL</sub>(p(x) || q(x)) ≠ D<sub>KL</sub>(q(x) || p(x)) (∴ violates the definition of mathematical distance)
...
>> ○ **Feature 1.** -∑ p<sub>i</sub> log q<sub>i</sub> = H(p) + D<sub>KL</sub>(p || q)

>> ○ **Feature 2.** Minimizing D<sub>KL</sub>(p || q) is the same as minimizing cross-entropy because H(p) is constant.
