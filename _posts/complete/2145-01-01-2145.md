...
>> ○ Context-dependent, one may also begin the derivation by writing the following, and this form is often regarded as the standard in practice.

<br>

$$
\log p(x) ;=; \log!\left(\mathbb{E}_{q(z\mid x)}!\left[\frac{p(x,z)}{q(z\mid x)}\right]\right),
$$

<br>

>> ○ ELBO(x) = 𝔼<sub>q(z)</sub> [log p(x,z)] - 𝔼<sub>q(z)</sub> [log q(z)]

>> ○ ELBO = 𝔼<sub>q(zㅣx)</sub> [log p(x,z)] - 𝔼<sub>q(zㅣx)</sub> [log q(zㅣx)] is the more official form.
...
>> ○ p(x,z) / q(z) = constant with respect to z ⇔ q(z) ∝ p(x,z) ⇔ q(z) = q(zㅣx) = p(x,z) / p(x) = p(zㅣx) 

>> ○ **Conclusion:** Maximizing the ELBO makes the variational distribution q(zㅣx) close to the true posterior p(zㅣx).
