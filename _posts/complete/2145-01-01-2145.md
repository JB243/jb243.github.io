...
<span style="color: #ffffff; background-color: #ee2323; font-family: Times New Roman;"><b>►</b></span> [Variational Inference Lecture](https://www.youtube.com/watch?v=x4I_4tDkN2o) 
...
> ① Another measure based on information theory, along with Shannon entropy. D = ∑<sub>i</sub> p<sub>i</sub><sup>2</sup>
...
<img width="543" height="369" alt="스크린샷 2026-06-15 오후 6 51 30" src="https://github.com/user-attachments/assets/0504dcc3-848d-4e5d-854f-9c080d764977" />

<br>

>> ○ ELBO(x) = 𝔼<sub>q(zㅣx)</sub>[log p(x,z)] - 𝔼<sub>q(zㅣx)</sub>[log q(zㅣx)]

>> ○ In variational inference, maximizing the ELBO makes the inequality approach equality.

>> ○ When the equality is established $p(x,z)/q(z \mid x)$ becomes constant with respect to $z$.

>> ○ $p(x,z) / q(z \mid x)$ = constant with respect to $z$ ⇔ $q(z \mid x) \propto p(x,z) = p(z \mid x)p(x) \propto p(z \mid x)$

>> ○ **Conclusion:** Maximizing the ELBO makes the variational distribution $q(z \mid x)$ close to the true posterior $p(z \mid x)$.

>> ○ For VAE, $\log p(x \mid z)$ is typically represented as the MSE between the original input $x$ and the reconstructed $\hat{x}$, assuming a Gaussian distribution.
