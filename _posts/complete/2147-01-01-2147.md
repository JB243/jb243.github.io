...
⑴ **Case 1.** Operating with Linux as the operating system, such as Ubuntu

⑵ **Case 2.** [Installing Linux on M1 MacBook with Parallels](https://cpuu.postype.com/post/9184363)

> ① M1 MacBook requires using a 64-bit ARM desktop image, not the AMD64 desktop image

> ② [Ryzen 7 does not support AMD-V enabling](https://forums.tomshardware.com/threads/which-ryzen-7-supports-amd-v-instruction.3702486/) **:** You might need to use the ARM desktop image

> ③ [Uninstalling Parallels on MacBook](https://it-talktalk.tistory.com/1211)

⑶ **Case 3.** [Installing Linux on Windows with VMware](https://www.vmware.com/kr/products/workstation-player/workstation-player-evaluation.html)

> ① Install Linux within a virtual environment

> ② File transfer can be done using mobaXterm

> ③ AMD-V enable might need to be explicitly enabled

>> ○ To explicitly enable AMD-V on GIGABYTE:

>> ○ Example 1

>>> ○ 1<sup>st</sup>. Quickly press the Del key multiple times until the BIOS screen appears after turning on the PC

>>> ○ 2<sup>nd</sup>. Go to the 'M.I.T.' tab, then navigate to 'Advanced Frequency Settings'

>>> ○ 3<sup>rd</sup>. Navigate to 'Advanced CPU Core Settings'

>>> ○ 4<sup>th</sup>. Switch from 'Disabled' to 'Enabled' for 'SVM Mode'

>> ○ Example 2

>>> ○ 1<sup>st</sup>. Go to the BIOS screen

>>> ○ 2<sup>nd</sup>. Go to the 'Tweaker' tab, then navigate to 'Advanced CPU Settings'

>>> ○ 3<sup>rd</sup>. Switch from 'Disabled' to 'Enabled' for 'SVM Mode'
...
○ `lshw | less` **:** View system specifications like CPU, RAM.

○ `lshw -C display` **:** View GPU information.
