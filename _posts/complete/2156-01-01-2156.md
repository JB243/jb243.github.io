...
```python
source("https://github.com/JB243/nate9389/blob/main/bin/A_Collection_of_Useful_Functions_in_RStudio.R?raw=true")
```
...
gene_starting_with <- function(keyword){
  human = read.csv("https://github.com/JB243/nate9389/blob/main/bin/human_genes_36601.tsv?raw=true", sep = '\t', header = F)
  mouse = read.csv("https://github.com/JB243/nate9389/blob/main/bin/mouse_genes_32285.tsv?raw=true", sep = '\t', header = F)
  ar = array()
  flag = 0
...
>> ○ [human_genes_36601.tsv](https://github.com/JB243/nate9389/blob/main/bin/human_genes_36601.tsv?raw=true)

>> ○ [mouse_genes_32285.tsv](https://github.com/JB243/nate9389/blob/main/bin/mouse_genes_32285.tsv?raw=true)
...
mouse_gene_to_MGI <- function(mouse_gene_list){
  ar = array(dim = length(mouse_gene_list))
	...
	dat <- read.csv("https://github.com/JB243/nate9389/blob/main/bin/HOM_MouseHumanSequence.csv?raw=true")
...
>> ○ [HOM_MouseHumanSequence.csv](https://github.com/JB243/nate9389/blob/main/bin/HOM_MouseHumanSequence.csv?raw=true)

<br>

```python
human_to_mouse <- function(human_gene){
  hom <- read.csv("https://github.com/JB243/nate9389/blob/main/bin/HOM_MouseHumanSequence.csv?raw=true")
...
⑹ gene name to chromosome position: Use of the conversion table([human](https://raw.githubusercontent.com/JB243/nate9389/refs/heads/main/bin/human_gene_annotation.csv), [mouse](https://github.com/JB243/nate9389/blob/main/bin/mouse_gene_annotation.csv?raw=true))
...
gene_to_chromosome_position <- function(gene_list){
  ...
  human <- read.csv("https://raw.githubusercontent.com/JB243/nate9389/refs/heads/main/bin/human_gene_annotation.csv")
  mouse = read.csv("https://github.com/JB243/nate9389/blob/main/bin/mouse_gene_annotation.csv?raw=true")
...
> ② When Seurat cannot be installed on a Linux server, you can try installing SeuratObject instead.
...
seurat_obj &lt;- readRDS("MuPSA_share.rds")
...
> ① Considering background genes (ver.1)

<br>

```python
GO.plot <- function(gene, bg_gene){
    library(clusterProfiler)
    library(org.Hs.eg.db)
    library(org.Mm.eg.db)
    library(enrichplot)
    ...
    bg_gene <- gsub('.*-', '', bg_gene)
    bg_gene <- gsub('\\ .*$', '', bg_gene)
    
    if (gene[1] == toupper(gene[1])){  ## Human gene
        gene.df <- bitr(gene, fromType = "SYMBOL", toType = "ENTREZID", OrgDb = org.Hs.eg.db)
        bg.df <- bitr(bg_gene, fromType = "SYMBOL", toType = "ENTREZID", OrgDb = org.Hs.eg.db)
        
        gene.entrez <- unique(gene.df$ENTREZID)
        bg.entrez <- unique(bg.df$ENTREZID)
        ...
        gene.df <- bitr(gene, fromType = "SYMBOL", toType = "ENTREZID", OrgDb = org.Mm.eg.db)
        bg.df <- bitr(bg_gene, fromType = "SYMBOL", toType = "ENTREZID", OrgDb = org.Mm.eg.db)
...
> ② Considering background genes (ver.2)
...
# 1. Bring up background genes
bg_df <- read.csv("~/Downloads/var.csv", check.names = FALSE)
colnames(bg_df)[1] <- "gene_symbol"
background_genes <- bg_df %>%  
  filter(feature_types == "Gene Expression") %>%  
  pull(gene_symbol) %>%  
  unique() %>%  
  na.omit()
...
# Use only the genes that exist in the background
top30_genes_use <- intersect(top30_genes, background_genes)
cat("Input top genes:", length(top30_genes), "\n")
cat("Genes found in background:", length(top30_genes_use), "\n")
cat("Background genes:", length(background_genes), "\n")
# Check the missing gene
setdiff(top30_genes, background_genes)

# 3. GO enrichment
ego <- enrichGO(
  gene          = top30_genes_use,
...
p <- dotplot(
  ...
  ggtitle("GO Biological Process enrichment: Top 30 genes") +
  theme_bw()
print(p)
```
