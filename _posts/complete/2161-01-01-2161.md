...
>> ○ **Definition:** A classification approach that finds a hyperplane capable of separating an N-dimensional space into two (N-1)-dimensional half-spaces.

>> ○ A traditional binary classification technique that sets an optimal hyperplane—called the maximum margin hyperplane (MMH)—by maximizing the margin in a high-dimensional (or even infinite-dimensional) space, thereby separating data points into two classes.
...
>>> ○ Frequently used method with well-established research.

>>> ○ Strong resistance to overfitting compared to other models.

>>> ○ High accuracy.

>>> ○ Generalizable irregardless of dimension.

>>> ○ Immune to noise.

>> ○ **Disadvantages**

>>> ○ Supervised learning: requires a labeled dataset (i.e., a provided training/test set).

>>> ○ Computationally intensive: slower than other models.

>>> ○ Kernel selection is not straightforward, so experimentation is necessary.
...
<img width="352" height="227" alt="스크린샷 2025-12-16 오후 3 15 25" src="https://github.com/user-attachments/assets/4a42d015-2d0a-4e71-80f7-3fe9bc1066e0" />

**Figure 3.** Decision Tree composed of a binary tree
...
>> ○ **Disadvantages 1.** Potential for overfitting, meaning it is vulnerable to outliers.

>> ○ **Disadvantages 2.** The tree becomes too deep.

>> ○ **Disadvantages 3.** Difficult to handle continuous numerical data

>> ○ **Disadvantages 4.** Difficulties in optimization
...
**Figure 5.** Structure of a K-D Tree
...
>> ○ An algorithm that combines prior information about classes with information extracted from data and uses Bayes’ theorem to classify whether a given data point belongs to a specific class. 

>> ○ Assumes conditional independence among features in the data. 

>> ○ Processes large-scale data quickly. 

>> ○ Can be used as a solution for text classification tasks, where documents are judged as belonging to one of multiple categories (e.g., spam, economy, sports, sentiment analysis, recommender systems, etc.) based on Bayesian estimation.
