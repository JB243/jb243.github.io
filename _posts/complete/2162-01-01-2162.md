...
class HMM(object):
	...
	def viterbi(self, sequence):
        ...
        maximal_state_probabilities = {}        

        def _init(cur_state):
            return self._initial[cur_state]

        def _emit(cur_state, symbol):
            return self._emissions[cur_state][symbol]

        def _transition(cur_state, next_state):
            return self._transitions[cur_state][next_state]

        
        # Stage 1. Initialization
        for state in hidden_states:
            maximal_state_probabilities[state] = [0.0] * len(sequence) 
        ...
        for symbol in sequence:
            for state in hidden_states:
                if idx == 0:
                    maximal_state_probabilities[state][idx] = _init(state) * _emit(state, symbol)
                else:
                    val = 0
                    for k in hidden_states: # k is previous state
                        val_candidate = maximal_state_probabilities[k][idx-1] * _transition(k, state) * _emit(state, symbol)
                        if val < val_candidate:
                            val = val_candidate
                        maximal_state_probabilities[state][idx] = val
            
            idx += 1
                        
        # Stage 3. Termination
        states_list = list(hidden_states)
        final_probs = [maximal_state_probabilities[state][len(sequence)-1] for state in hidden_states]
        best_final_state_idx = np.argmax(final_probs)
        best_final_state = states_list[best_final_state_idx]
        ...
        while idx >= 0:
            ...
            for k in hidden_states:
                val_candidate = (
                    maximal_state_probabilities[k][idx-1]
                    * _transition(k, state)
                    * _emit(state, symbol)
                )
                if abs(val_candidate - maximal_state_probabilities[state][idx]) < 1e-15:
                    state = k
                    break
 ...
 **Figure 2.** Agent-Environment Interaction in MDP
