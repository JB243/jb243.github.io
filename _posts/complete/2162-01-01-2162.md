...
>> ○ **Step 3.** Observe the next state $x'$ and receive $R(x', u)$ from the environment

>> ○ **Step 4.** Update $Q̂(x, u)$

> ③ Supplements

>> ○ Learning the full model requires $\left\|\mathcal{S}|^2|\mathcal{A}\right\|$ memory, but Q-learning only needs $\left\|\mathcal{S}|\times|\mathcal{A}\right\|$ memory.
...
>> ○ **Step 3.** Observe the next state x' and receive R(x', u)
