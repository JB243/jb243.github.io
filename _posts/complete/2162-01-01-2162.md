...
>>> ○ Definition: Train two Q-functions (with parameters $w$ and $w'$), then take their average to mitigate the overestimation problem. In this setup, the target network is used to select the action, and the online network is used to evaluate its value.
