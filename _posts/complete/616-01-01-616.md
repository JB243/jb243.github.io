...
>> ○ **Assumption 1.** Exogeneity: system variables do not cause intervention variables.

>> ○ **Assumption 2.** Generic context: The intervention variables are fully connected.
...
>>> Let $G_I$ denote the DAG $G$ together with additional $I$-vertices $｛\zeta_k\｝_{k\in [K]}$ and $I$-edges $｛\zeta_k \to i｝_{i\in I_k, I_k\in \mathcal{I}}.$$ If $｛f_k｝_{k\in [K]}$ is $I$-Markov with respect to $G$, then for disjoint $A$ and $C$,
