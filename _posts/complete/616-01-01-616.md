...
⑵ Degree of Graph G: Number of nodes, i.e., $\left\|V(G)\right\|$

⑶ Size of Graph G: Number of edges, i.e., $\left\|E(G)\right\|$
...
⑿ Degree distribution: The ratio of nodes with degree k among all nodes, i.e., P(k) = N<sub>k</sub> / ㅣV(G)ㅣ.
...
> ① Directed Acyclic Graph (DAG) is often used in causal inference.
...
> ② Learning a DAG that represents a causal structure from data is NP-hard.

>> ○ Typically approximated via greedy search over the DAG space (or the Markov equivalence class space).

>> ○ Because the DAG space/equivalence class space is extremely large, the paper argues that permutation-based greedy search—searching in the smaller permutation space—is attractive and formalizes this approach.

>> ○ The search proceeds in a direction that yields a “sparser” DAG.
...
>> ○ Example: If $X_2 = X_1 + \varepsilon$ (equivalently, $X_1 = X_2 - \varepsilon$), then $G_1: X_1 \to X_2$ and $G_2: X_1 \leftarrow X_2$ cannot be distinguished.
...
>> ○ Intervention = Perturbation (often used in biology)

>> ○ An intervention changes only the value of a node without altering the graph structure.

>> ○ Perfect intervention 

>> ○ Imperfect intervention 

> ② I-Markov Property given $I$ (Invariance)
...
$$
f(x) = \prod_{i \notin I} f^{\mathrm{obs}}\left(x_i \mid x_{\mathrm{pa}_{G(i)} }\right) \prod_{i \in I} f^{I}\left(x_i \mid x_{\mathrm{pa}_G(i)}\right)
$$
...
> ③ I-MEC: A subset of MEC.

⑹ Causal Inference

> ① Constraint-based method (e.g., PC)

>> ○ Learn the causal model by treating causal inference as a constraint satisfaction problem and estimate the underlying Markov equivalence class by a sequence of conditional independence tests.

> ② Score-based method (e.g., GES, GIES)

>> ○ Assign a score to each Markov equivalence class and learn the Markov equivalence class of the data generating DAG by greedily optimizing a penalized likelihood score.

>> ○ Because it assumes a specific probability distribution, the statistical power is reduced.

> ③ Hybrids (e.g., GSP, IGSP)

>> ○ Construct a score function based on conditional independence tests.

> ④ [Permutation-Based Causal Structure Learning](https://arxiv.org/abs/1910.09007)

>> ○ **Goal 1.** To identify intervention targets.

>> ○ **Goal 2.** To reconstruct the causal graph: without assumptions, there is no identifiability.

>>> ○ Consider I-essential (a partially directed graph) instead of I-MEC, because the direction may not be identifiable.

>> ○ **Assumption 1.** DAG (directed acyclic graph)

>>> ○ **Counterexample:** EGFR ligands such as TGF-α and HB-EGF can be induced by EGFR signaling, forming a positive feedback mechanism; therefore, even this assumption does not hold biologically.

>> ○ **Assumption 2.** Exogeneity: system variables do not cause intervention variables.

>> ○ **Assumption 3.** Generic context: The intervention variables are fully connected.

>> ○ **Assumption 4.** Direct I-faithfulness (strong assumption): the equality condition and the intervention are linked as necessary and sufficient conditions (i.e., “if and only if”).

<br>

<img width="377" height="87" alt="스크린샷 2026-03-02 오후 7 56 58" src="https://github.com/user-attachments/assets/7bdaf3b6-ce9b-4a5e-b2e5-20573d7aeb6b" />

<br>

>> ○ **Assumption 5.** I-faithfulness assumption: Let $\mathcal{I}$ be a list of intervention targets. The set of distributions $\{f_{\mathrm{obs}}\}\cup\{f_I\}_{I\in\mathcal{I}}$ is $I$-faithful with respect to a DAG $G$ if $f_{\mathrm{obs}}$ is faithful with respect to $G$ and for any $I_k\in\mathcal{I}$ and disjoint $A,C\subseteq [p]$, we have that:

$$
\left( A \perp \zeta_k \mid C \cup \zeta_{[K]\setminus\{k\}} \right)_{G^I}
\iff
f_k(x_A \mid x_C)=f_{\mathrm{obs}}(x_A \mid x_C)
$$

>> ○ **Assumption 6.** Sparse permutation principle: the permutation with the fewest edges is assumed to reflect the true underlying structure; this is akin to Occam’s razor.

<br>

<img width="424" height="481" alt="스크린샷 2026-03-02 오후 7 39 54" src="https://github.com/user-attachments/assets/13636740-2772-4716-9093-ff46d9512ba6" />

**Figure 7.** Example of the assumed causal graph 

<br>

>> ○ JCI (Joint Causal Inference): With data $x$, define the likelihood of a candidate causal model as $f^{\mathrm{joint}}(x, I)$.

<bR>

<img width="544" height="117" alt="스크린샷 2026-03-02 오후 7 48 06" src="https://github.com/user-attachments/assets/6398d392-838f-4a4e-9849-469b5b296231" />

<br>

>> ○ Identifiability theorem for **unknown target** identification.

>>> Let $G_I$ denote the DAG $G$ together with additional $I$-vertices
...
$$
{\zeta_k \to i}_{i\in I_k, I_k\in \mathcal{I}}.
$$

<br>

>>> If ${f_k}_{k\in [K]}$ is $I$-Markov with respect to $G$, then for disjoint $A$ and $C$, where $C$ is intervention target,

<br>

$$
f_I(x_A \mid x_C) = f_{\mathrm{obs}}(x_A \mid x_C)
$$

<br>

>>> whenever $A$ and $\zeta_I$ are d-separated given $C \cup \zeta_{\mathcal{I}\setminus \mathcal{I}}$, denoted as

<br>

$$
\left( A \perp \zeta_I \mid C \cup \zeta_{\mathcal{I}\setminus \mathcal{I}} \right)_{G_I}.
$$

<br>

>> ○ Interventional Greedy SP (IGSP)

>>> ○ Input: $f_{\text{obs}}, f_1, \cdots, f_k, I = \{I_1, I_2, \cdots, I_k\}$ (fully unknown or partially known)

>>> ○ Output: $\pi$, I-MAP, $I = \{I_1, \cdots, I_k\}$ (true target)

>>> ○ Objective function

<br>

<img width="385" height="153" alt="스크린샷 2026-03-02 오후 10 45 07" src="https://github.com/user-attachments/assets/01ede655-8d57-4e4f-80d0-2686979ef918" />

<br> 

>>> ○ Permutation includes intervention variables.
...
⑺ [The Soccer Ball and Euler’s Formula](https://jb243.github.io/pages/1767)
...
⑾ [Four color theorem](https://jb243.github.io/pages/1544)
