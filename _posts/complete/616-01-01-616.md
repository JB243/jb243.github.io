...
>> ○ Goal: to handle the case where the intervention target is unknown.
...
>>> ○ With data $x$, define the likelihood of a candidate causal model as $f^{\mathrm{joint}}(x, I)$.
...
>>> ○ $I_k$: a one-hot encoding vector indicating one perturbation (intervention) experiment.
...
>>> If ${f_k}_{k\in [K]}$ is $I$-Markov with respect to $G$, then for disjoint $A$ and $C$,
