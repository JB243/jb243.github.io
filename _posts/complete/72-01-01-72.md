...
>> ○ De novo methylation: In all genes during gametogenesis, methylation is erased and re-established according to whether the gene comes from a paternal or maternal origin. 
...
>> ○ Uniparental reproduction is prevented by differences in imprinted genes present in the genomes of the egg and sperm.
...
>> ○ Chloroplast inheritance: The following is an example using *Chlamydomonas* (a haploid, unicellular green alga).

>>> ○ *Chlamydomonas* has two mating types, mt<sup>+</sup> and mt<sup>−</sup>, which are determined by genes in the nucleus.

>>> ○ When mt<sup>+</sup> and mt<sup>−</sup> mate, they form a zygote (2n), and the zygote undergoes meiosis to produce four offspring (n).

>>> ○ Only the chloroplasts from the mt<sup>+</sup> type are transmitted to the offspring.
...
> ⑥ [Sex chromosomal aneuploidy](https://jb243.github.io/pages/71#5-mutation)
