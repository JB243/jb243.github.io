...
The function V<sub>t</sub>(x) decreases as t increases, and increases as x increases. Therefore, for a given time t, we can show that the incentive to search increases as x becomes larger, and that it is optimal to search only when x ≥ x<sub>t</sub>\* When there is less time remaining, the expected gain is not very large, so one prefers gains that can be secured in the short term, and the motivation to search becomes stronger. In other words, x<sub>t</sub>\* ≥ x<sub>t+1</sub>\*. This tendency is clearly seen when we define S<sub>t</sub><sup>weak</sup> := {x ㅣ Q<sub>t</sub>(x, stay) ≥ Q<sub>t</sub>(x, look)}. However, when we take S<sub>t</sub><sup>strict</sup> := {x ㅣ Q<sub>t</sub>(x, stay) ≥ Q<sub>t</sub>(x, look)}, we obtain x<sub>t</sub>\* = x<sub>t+1</sub>\*, which is also evidence that this tendency is not very strong. Note that we can show x<sub>N-1</sub>\* = x<sub>N</sub>\*, V<sub>N-1</sub>(x) = V<sub>N</sub>(x) ∀x ≤ x<sub>N</sub>\*, and we can intuitively understand this induction assumption is maintained by symmetry.
