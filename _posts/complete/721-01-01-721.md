...
See [this link](https://www.youtube.com/watch?v=d1VkSkt0EbY&list=PLGboZ4litMr_TOwUANH-s-uFnczzy2uuW&index=7).
...
See this [link](https://www.youtube.com/watch?v=oGodjYGcy5A&list=PLGboZ4litMr_TOwUANH-s-uFnczzy2uuW&index=8&pp=iAQB).

This system shows a time-invariant characteristics. Thus, the Bellman equation is as follows:

V<sub>t</sub>(x) = max{x, p𝔼[V<sub>t+1</sub>(X<sub>t+1</sub> ㅣ x)]} = max{x, p𝔼[V<sub>t+1</sub>(X<sub>t+1</sub>)]} = max{x, a}, where a is a constant.

Therefore, the optimal policy is as follows: if x ≥ a, buy it; otherwise, reject it.

Here, we can obtain the following fixed-point equation: a = ∫<sub>-∞ to a</sub> af(y)dy + ∫<sub>a to ∞</sub> yf(y) dy ⇔ 0 = ∫<sub>a to ∞</sub> (y - a)f(y) dy.
