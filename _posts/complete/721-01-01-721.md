...
See [this link](https://www.youtube.com/watch?v=d1VkSkt0EbY&list=PLGboZ4litMr_TOwUANH-s-uFnczzy2uuW&index=7). The expected reward becomes larger as t gets closer to 0, and the price p<sub>t</sub> at time t should be set as: p<sub>t</sub> = ((expected gain at t+1) + 1​) / 2.
...
V<sub>t</sub>(x) = max{x, p𝔼[V<sub>t+1</sub>(X<sub>t+1</sub> ㅣ x)]} = max{x, p𝔼[V<sub>t+1</sub>(X<sub>t+1</sub>)]} = max{x, a},
...
a = ∫<sub>-∞ to a</sub> af(y)dy + ∫<sub>a to ∞</sub> yf(y) dy ⇔ 0 = ∫<sub>a to ∞</sub> (y - a)f(y) dy.
