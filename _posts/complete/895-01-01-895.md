...
> ① In general, ℙ(x<sub>t+2</sub><sup>g</sup> ∈ A | x<sub>t</sub>, u<sub>t</sub>, u<sub>t+1</sub>) ≠ ℙ(x<sub>t+2</sub><sup>g</sup> ∈ A | x<sub>0:t</sub>, u<sub>0:t+1</sub>)
...
<img width="552" height="240" alt="스크린샷 2025-10-05 오전 10 01 48" src="https://github.com/user-attachments/assets/6f446d79-9a1c-49e8-9cf4-28cd2de53b4f" />

<br>

>> ○ stability of A is a **sufficient**, but **not necessary** condition: ∑<sub>∞</sub> may still exist uniquely even if A is not stable
...
> ④ stationary state: If Pr(x<sub>n</sub> | x<sub>n-1</sub>) is independent of n, the Markov process is stationary (time-invariant)
...
>>> ○ P<sub>ij</sub>: probability of transition from node j to node i. ∑<sub>i</sub> P<sub>ij</sub> = 1
...
<img width="1078" height="1238" alt="image" src="https://github.com/user-attachments/assets/73a9fdbf-078c-410b-8d86-60096a95bad7" />
...
>> ○ (Note) time-homogeneous: {x<sub>t</sub><sup>g</sup>}<sub>t≥0</sub> and {x<sub>t</sub><sup>g</sup>}<sub>t≥τ,∀τ∈ℤ<sup>+</sup></sub> follow the same distribution

>> ○ **Condition 1.** time-homogeneous transition: P<sub>t</sub>(j | i, u) = P(j | i, u) ∀t
...
> ① If Pg is not irreducible, the state space S splits into the transient states T and one or more recurrent communicating classes C<sub>1</sub>, ···
...
>> ○ The stationary distribution π<sup>g</sup> assigns probability 0 to all transient states.
...
>> Uniqueness is not trivial. Assuming the solution is bounded often allows one to show uniqueness. Consider the equation for the difference of two solutions U<sup>g</sup> = R<sup>g</sup>U<sup>g</sup>. Connecting this to the diagram yields U<sup>g</sup>(ℓ+1) - U<sup>g</sup>(ℓ) = (λ - 1)(U<sup>g</sup>(ℓ) - U<sup>g</sup>(ℓ-1)). The consecutive differences Δ(ℓ) = U<sup>g</sup>(ℓ+1) - U<sup>g</sup>(ℓ) form a geometric sequence with ratio (λ - 1). If |λ - 1| < 1, these differences converge to 0, suggesting a bounded solution. If |λ - 1| ≥ 1, the differences may diverge, implying that uniqueness may fail.
